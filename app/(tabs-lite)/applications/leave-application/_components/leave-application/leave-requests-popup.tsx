...
export default function LeaveApplicationRequestsPopup({
  isOpen,
  onClose,
  ...
  const collectionName = useMemo(() => {
    ...
    if (userMode === "approver") {
      if (workflowState?.includes("APPROVED") || workflowState?.includes("REJECTED") || workflowState?.includes("CANCELLED")) {
  ...
  const showActionControls = collectionName !== "leaveApplicationTransaction"
    && !isProcessed
      ...
      <View className="flex-1 bg-white">
        ...
        <View style={{ paddingHorizontal: 14, paddingTop: 58, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#e5e7eb', backgroundColor: '#fff' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <Text style={{ fontFamily: 'Inter', fontSize: 20, fontWeight: '700', color: '#0f172a' }}>Leave Application</Text>
          </View>
        </View>
            ...
            <View className="px-4 py-4">
              ...
              <View className="bg-white rounded-lg border border-gray-100 p-4 mb-3">
                ...
                {!!selectedRequest.uploadTime && (
                  <DetailRow label="Upload Time" value={formatDateTime(selectedRequest.uploadTime)} />
                )}
                {!!selectedRequest.appliedDate && (
                  <DetailRow label="Applied Date" value={formatDDMMYYYY(selectedRequest.appliedDate)} />
                )}
                {!!selectedRequest.remarks && (
                  <DetailRow label="Remarks" value={selectedRequest.remarks} />
                )}
                {!!selectedRequest.comment && (
                  <DetailRow label="Comment" value={selectedRequest.comment} />
                )}
              </View>
