...
// Serialise a UserEntitlement as an inline GQL argument string.
// The schema does not expose userEntitlement as a $variable, so it must
// be baked directly into the query string.
...
export function useLeavePolicy({
...
}): { leaveOptions: LeaveOption[]; loading: boolean } {
  const userEntitlement = useUserEntitlement()

  // Build both query strings with userEntitlement baked in as an inline arg
  ...
  const leaveCriteria = useMemo(() => {
    ...
    criteria.push(
      { field: 'tenantCode', operator: 'is', value: tenantCode },
      { field: 'leavePolicy.leaveCategory', operator: 'eq', value: leaveCategory }
    )
    ...
    return criteria
  }, [tenantCode, leaveCategory, deployment])
