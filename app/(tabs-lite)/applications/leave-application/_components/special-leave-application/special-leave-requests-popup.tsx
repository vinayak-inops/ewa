...
function mapBackendToPunchRequest(item: any): PunchRequest {
  return {
    ...
    typeOfAbsence: item.typeOfAbsence || "",
    leaves: Array.isArray(item.leaves) ? item.leaves : [],
    documents: Array.isArray(item.documents) ? item.documents : [],
    remarks: item.remarks || "",
    status: item.workflowState?.toLowerCase().includes("approved")
      ? "approved"
      : item.workflowState?.toLowerCase().includes("rejected")
        ? "rejected"
        : item.workflowState?.toLowerCase().includes("validated")
          ? "validated"
          : item.workflowState?.toLowerCase().includes("failed")
            ? "failed"
            : "pending",
...
export default function SpecialLeaveRequestsPopup({
  ...
  const collectionName = useMemo(() => {
    ...
    if (userMode === "approver") {
      if (
        workflowState?.includes("APPROVED") ||
        workflowState?.includes("REJECTED") ||
        workflowState?.includes("CANCELLED")
      ) {
  ...
  useEffect(() => {
    if (attendanceResponse && Array.isArray(attendanceResponse)) {
      const mapped = attendanceResponse.map(mapBackendToPunchRequest)
      setPunchRequests(mapped)
      if (selectedRequestId) {
        const found = mapped.find((r: PunchRequest) => r.id === selectedRequestId)
  ...
  const guessMime = (nameOrPath?: string, fallback?: string) => {
  ...
  const isProcessed = wfState === "APPROVED" || wfState === "REJECTED" || wfState === "CANCELLED" || wfState === "FAILED"
  const showActionControls = collectionName !== "specialLeaveApplicationTransaction"
    && !isProcessed
  ...
  const handleSubmitAction = () => {
    ...
    if (
      (statusAction === "approve" && !canApprove) ||
      (statusAction === "reject" && !canReject) ||
      (statusAction === "cancel" && !canCancel)
    ) {
  ...
  if (submitSuccess) {
    ...
    const subtitle = isApprove
      ? "The special leave request has been approved successfully."
      : isReject
      ? "The special leave request has been rejected."
      : "The special leave request has been cancelled."
              ...
              {selectedRequest && (
                <View style={{
                  ...
                  <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 14, rowGap: 14 }}>
                    {[
                      { label: "Leave Title", value: selectedRequest.leaveTitle || "—" },
      ...
      <View className="flex-1 bg-white">
        ...
        <View style={{ paddingHorizontal: 14, paddingTop: 58, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#e5e7eb', backgroundColor: '#fff' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <Text style={{ fontFamily: 'Inter', fontSize: 20, fontWeight: '700', color: '#0f172a' }}>Special Leave Application</Text>
          </View>
        </View>
          ...
          {selectedRequest ? (
            <View className="px-4 py-4">
              ...
              <View className="bg-white rounded-lg border border-gray-100 p-4 mb-3">
                ...
                {!!selectedRequest.leaveTitle && <DetailRow label="Leave Title" value={selectedRequest.leaveTitle} />}
                {!!selectedRequest.typeOfAbsence && <DetailRow label="Type" value={selectedRequest.typeOfAbsence} />}
                ...
                {!!selectedRequest.lastDayOfWork && <DetailRow label="Last Day of Work" value={formatDDMMYYYY(selectedRequest.lastDayOfWork)} />}
                {selectedRequest.noOfDays !== undefined && selectedRequest.noOfDays !== "" && (
                  <DetailRow label="No. of Days" value={String(selectedRequest.noOfDays)} />
                )}
                {!!selectedRequest.DOBOfChild && <DetailRow label="DOB of Child" value={formatDDMMYYYY(selectedRequest.DOBOfChild)} />}
                {!!selectedRequest.AdoptionPlacementDate && <DetailRow label="Adoption Date" value={formatDDMMYYYY(selectedRequest.AdoptionPlacementDate)} />}
                {!!selectedRequest.appliedDate && <DetailRow label="Applied Date" value={formatDDMMYYYY(selectedRequest.appliedDate)} />}
                {!!selectedRequest.uploadTime && <DetailRow label="Upload Time" value={formatDateTime(selectedRequest.uploadTime)} />}
              ...
              {Array.isArray(selectedRequest.leaves) && selectedRequest.leaves.length > 0 && (
                <View className="bg-white rounded-lg border border-gray-100 mb-3 overflow-hidden">
                  <Text className="text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 pt-3 pb-2">Leaves</Text>
                  <View className="flex-row bg-gray-50 border-t border-b border-gray-100 px-4 py-2">
                    <Text className="text-xs font-semibold text-gray-600 flex-1">Date</Text>
                    <Text className="text-xs font-semibold text-gray-600 flex-1">Leave Code</Text>
                    <Text className="text-xs font-semibold text-gray-600 flex-1">Duration</Text>
                  </View>
                  {selectedRequest.leaves.map((l, idx) => (
                    <View key={idx} className="flex-row px-4 py-2 border-b border-gray-50">
                      <Text className="text-sm text-gray-700 flex-1">{formatDDMMYYYY(l.date)}</Text>
                      <Text className="text-sm text-gray-700 flex-1">{l.leaveCode}</Text>
                      <Text className="text-sm text-gray-700 flex-1">{l.duration}</Text>
                    </View>
                  ))}
                </View>
              )}
              ...
              {Array.isArray(selectedRequest.documents) && selectedRequest.documents.length > 0 && (
                <View className="bg-white rounded-lg border border-gray-100 mb-3 overflow-hidden">
                  <Text className="text-xs font-semibold text-gray-500 uppercase tracking-wide px-4 pt-3 pb-2">Documents</Text>
                  {selectedRequest.documents.map((d, idx) => {
                    const serverPath = d.filePath || ""
                    const mime = d.fileType || guessMime(d.fileName || d.filePath)
                    const isImage = (mime || "").startsWith("image/")
                    return (
                      <View key={idx} className="border-t border-gray-100">
                        <View className="flex-row items-center justify-between px-4 py-3">
                          ...
                          <View className="flex-row gap-2">
                            {serverPath && (
                              <TouchableOpacity
                                onPress={async () => {
                                  if (docPreviewIdx === idx) { setDocPreviewIdx(null); setDocPreviewUrl(""); return }
                                  setDocPreviewIdx(idx); setDocPreviewUrl("")
                                  const res: any = await fetchByteArray(serverPath, mime || "application/octet-stream")
                                  if (res?.success && res.objectUrl) setDocPreviewUrl(res.objectUrl)
                                }}
                                className="px-2.5 py-1.5 border border-blue-200 rounded bg-blue-50"
                              >
                                <Text className="text-blue-600 text-xs font-medium">
                                  {docPreviewIdx === idx ? "Hide" : "Preview"}
                                </Text>
                              </TouchableOpacity>
                            )}
                            {serverPath && (
                              <TouchableOpacity
                                onPress={() => Linking.openURL(serverPath)}
                                className="px-2.5 py-1.5 border border-gray-200 rounded bg-white"
                              >
                                <Text className="text-gray-700 text-xs font-medium">Download</Text>
                              </TouchableOpacity>
                            )}
                          </View>
                        </View>
                        ...
                        {docPreviewIdx === idx && (
                          <View
                            className="mx-4 mb-3 rounded-md border border-gray-200 bg-gray-50 overflow-hidden"
                            style={{ minHeight: 100 }}
                          >
                            {docLoading ? (
                              <View className="items-center justify-center py-10 gap-2">
                                <ActivityIndicator size="small" color="#6b7280" />
                                <Text className="text-sm text-gray-500">Loading preview...</Text>
                              </View>
                            ) : docError ? (
                              <View className="items-center justify-center py-10">
                                <Text className="text-sm text-red-600">{docError}</Text>
                              </View>
                            ) : docPreviewUrl && isImage ? (
                              <Image
                              ...
                              <View className="items-center justify-center py-8 px-4 gap-3">
                                <Text className="text-sm text-gray-600 text-center">
                                  Preview not available for this file type.
                                </Text>
                                <TouchableOpacity
                                  onPress={() => Linking.openURL(docPreviewUrl)}
                              ...
                              <View className="items-center justify-center py-8">
                                <Text className="text-sm text-gray-400">No preview available</Text>
                              </View>
                            )}
                          </View>
                        )}
                  ...
                  {statusAction && (
                    <TouchableOpacity
                      ...
                      className={`h-11 rounded-lg items-center justify-center flex-row gap-2 ${
                        statusAction === "cancel" ? "bg-gray-900"
