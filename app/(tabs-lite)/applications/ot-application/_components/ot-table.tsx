...
const TAB_DESC: Record<OtTabKey, string> = {
  all:       "Showing all OT / out duty applications.",
...
const SEARCH_FIELDS = [
  ...
  { value: "outDutyType",   label: "Duty Type",   icon: "📋" },
  { value: "fromDate",      label: "From Date",   icon: "📅" },
  { value: "toDate",        label: "To Date",     icon: "📅" },
  { value: "Reason",        label: "Reason",      icon: "💬" },
...
export default function OtTable({
  ...
  activeTab, onTabChange, onNew, title = "OT / Out Duty Applications", subtitle,
}: Props) {
                ...
                <React.Fragment key={row._id || idx}>
                  ...
                  >
                    ...
                    <View style={{ flex: 1, gap: 2 }}>
                      <Text style={{ fontSize: 13, fontWeight: "600", color: "#0f172a" }} numberOfLines={1}>{row.employeeID || "OT Request"}</Text>
