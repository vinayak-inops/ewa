...
import { ChevronLeft, ChevronRight, Clock, FileText } from "lucide-react-native"
...
export type OtTabKey = "all" | "pending" | "approved" | "rejected" | "cancelled" | "failed"
...
interface Props {
  data: OtRecord[]
  onOpenDetails: (row: OtRecord) => void
  loading?: boolean
...
export default function OtTable({
  ...
  activeTab, onTabChange, onNew, title = "OT / Out Duty Applications",
    ...
    <View style={{ flex: 1, backgroundColor: "#f8fafc" }}>
      ...
      <View style={s.banner}>
        <View style={s.bannerText}>
          <Text style={s.bannerTitle}>Extra Hours? Get Recognized.</Text>
          <Text style={s.bannerSub}>Submit & track overtime requests instantly</Text>
          {/* <Pressable style={s.bannerLink}>
            <Text style={s.bannerLinkText}>Learn More →</Text>
          </Pressable> */}
        </View>
        <View style={s.bannerIcon}>
          <Clock size={38} color="rgba(255,255,255,0.25)" />
        </View>
      </View>
                      ...
                      <View style={{ flex: 1, gap: 2 }}>
                        <Text style={{ fontSize: 13, fontWeight: "700", color: "#0f172a" }} numberOfLines={1}>
                          {row.employeeID && row.employeeName
                            ? `${row.employeeID} - ${row.employeeName}`
                            : row.employeeName || row.employeeID || "OT Request"}
                        </Text>
