...
export default function PunchRequestsPopup({ isOpen, onClose, initialSelectedRequest, selectedRequestId, userMode = "user", sourceCollectionName, onActionSuccess }: Props) {
  ...
  const { post: postAction } = usePostRequest<any>({
    url: collectionName,
    onSuccess: () => { refetch(); onActionSuccess?.(); setSubmitSuccess(statusAction); setStatusLoading(false); setStatusAction(null); setStatusComment(""); setStatusError("") },
  ...
  const showActionControls = collectionName !== "forgotPunchApplicationTransaction" && !isProcessed && (canApprove || canReject || canCancel)
  ...
  if (submitSuccess) {
    ...
    const subtitle = isApprove
      ? "The punch request has been approved successfully."
      : isReject
      ? "The punch request has been rejected."
      : "The punch request has been cancelled."
              ...
              {selectedRequest && (
                <View style={{
                  ...
                  <View style={{ flexDirection: "row", flexWrap: "wrap", marginTop: 14, rowGap: 14 }}>
                    {[
                      { label: "Attendance Date", value: formatDate(selectedRequest.attendanceDate) },
                      { label: "Punch Time",       value: formatTime(selectedRequest.punchedTime) },
                      { label: "In / Out",          value: selectedRequest.inOut || "—" },
      ...
      <View className="flex-1 bg-white">
        ...
        <View style={{ paddingHorizontal: 14, paddingTop: 58, paddingBottom: 12, borderBottomWidth: 1, borderBottomColor: '#e5e7eb', backgroundColor: '#fff' }}>
          <View style={{ flexDirection: 'row', alignItems: 'center', gap: 10, marginBottom: 4 }}>
            ...
            <Text style={{ fontFamily: 'Inter', fontSize: 20, fontWeight: '700', color: '#0f172a' }}>Punch Application</Text>
          </View>
        </View>
          ...
          {selectedRequest ? (
            <View className="px-4 py-4">
              ...
              <View className="bg-white rounded-lg border border-gray-100 p-4 mb-3">
                ...
                {!!selectedRequest.attendanceDate && <DetailRow label="Attendance Date" value={formatDate(selectedRequest.attendanceDate)} />}
                {!!selectedRequest.inOut && <DetailRow label="In / Out" value={selectedRequest.inOut} />}
                {!!selectedRequest.typeOfMovement && <DetailRow label="Movement Type" value={selectedRequest.typeOfMovement} />}
                {!!selectedRequest.punchedTime && <DetailRow label="Punch Time" value={formatTime(selectedRequest.punchedTime)} />}
