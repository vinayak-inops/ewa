...
import { ChevronLeft, ChevronRight, FileText, Wallet } from "lucide-react-native"
...
interface Props {
  ...
  onOpenDetails: (row: EncashmentRecord) => void
  loading?: boolean
...
export default function EncashmentTable({
  data, onOpenDetails, loading = false, externalPagination,
  onNew, title = "Encashment Applications",
    ...
    <View style={{ flex: 1, backgroundColor: "#f8fafc" }}>
      ...
      <View style={s.banner}>
        <View style={s.bannerText}>
          <Text style={s.bannerTitle}>Turn Leave into Value.</Text>
          <Text style={s.bannerSub}>Submit & track leave encashment requests instantly</Text>
          {/* <Pressable style={s.bannerLink}>
            <Text style={s.bannerLinkText}>Learn More →</Text>
          </Pressable> */}
        </View>
        <View style={s.bannerIcon}>
          <Wallet size={38} color="rgba(255,255,255,0.25)" />
        </View>
      </View>
          ...
          <View style={{ paddingHorizontal: 14, paddingBottom: 14 }}>
            {loading ? (
              ...
              currentData.map((row, idx) => {
                ...
                const subLine = [row.leaveCode, row.balance != null ? `${row.balance} days` : ""].filter(Boolean).join("  ·  ")
                const detail = row.remarks || ""
                return (
                      ...
                      <View style={{ alignItems: "flex-end", flexShrink: 0, gap: 4 }}>
                        <View style={{ borderRadius: 999, borderWidth: 1, paddingHorizontal: 8, paddingVertical: 2, backgroundColor: st.bg, borderColor: st.border }}>
                          <Text style={{ fontSize: 10, fontWeight: "700", color: st.text }}>
                            {(row.workflowState || "PENDING").toUpperCase()}
                          </Text>
                        </View>
                        <Text style={{ fontSize: 11, color: "#94a3b8" }}>{formatDate(row.createdOn)}</Text>
                      </View>
                    </Pressable>
