...
function getRemainingCompOff(tx: Partial<CompOffTransaction>) {
  return Math.max((Number(tx.calculatedCompOff) || 0) - (Number(tx.availedCompOff) || 0), 0)
}

// ── Helpers ───────────────────────────────────────────────────────────────────

function validate(fields: { fromDate: string; toDate: string }): FormErrors {
  const e: FormErrors = {}
  if (!fields.fromDate) e.fromDate = "From date is required"
  if (!fields.toDate) {
    e.toDate = "To date is required"
  } else if (fields.fromDate && new Date(fields.toDate) < new Date(fields.fromDate)) {
...
function TransactionPickerModal({ visible, onClose, transactions, loading, selectedDates, onToggle, onSelectAll, onClearAll }: {
  visible: boolean; onClose: () => void
  transactions: CompOffTransaction[]; loading: boolean
      ...
      <View style={s.sheetBg}>
        <View style={s.sheet}>
          ...
          <View style={s.sheetHead}>
            <View>
              <Text style={s.sheetTitle}>Available Comp-Off Dates</Text>
              <Text style={s.sheetSub}>{selectedCount > 0 ? `${selectedCount} selected` : "Tap to select"}</Text>
            </View>
            <TouchableOpacity onPress={onClose} hitSlop={{ top: 8, bottom: 8, left: 8, right: 8 }}>
          ...
          <View style={{ flexDirection: "row", justifyContent: "flex-end", gap: 12, paddingHorizontal: 16, paddingVertical: 8, borderBottomWidth: 1, borderBottomColor: "#f1f5f9" }}>
            <TouchableOpacity onPress={onSelectAll}>
              <Text style={{ fontFamily: FONT, fontSize: 12, color: NAVY, fontWeight: "600" }}>Select All</Text>
            </TouchableOpacity>
            <TouchableOpacity onPress={onClearAll}>
              <Text style={{ fontFamily: FONT, fontSize: 12, color: "#64748b" }}>Clear All</Text>
            </TouchableOpacity>
          </View>
          ...
          {loading ? (
            <View style={{ paddingVertical: 32, alignItems: "center" }}>
              ...
              <Text style={[s.muted, { marginTop: 6 }]}>Loading transactions...</Text>
            </View>
          ) : transactions.length === 0 ? (
            <View style={{ paddingVertical: 32, alignItems: "center", paddingHorizontal: 24 }}>
              <Ionicons name="calendar-outline" size={32} color="#cbd5e1" />
              <Text style={[s.muted, { marginTop: 8, textAlign: "center", fontSize: 13 }]}>No comp-off transactions available</Text>
              <Text style={[s.muted, { marginTop: 4, textAlign: "center", fontSize: 11 }]}>Enter an employee ID to load available transactions</Text>
            </View>
          ) : (
            <FlatList
              data={transactions}
              ...
              style={{ maxHeight: 340 }}
              renderItem={({ item }) => {
                const sel = selectedDates.includes(item.date)
                const remaining = getRemainingCompOff(item)
                return (
                  <TouchableOpacity
                    onPress={() => onToggle(item.date)}
                    style={[s.txRow, sel && { backgroundColor: "#f0f4ff" }]}
                  >
                    <View style={{ flex: 1 }}>
                      <Text style={[s.txDate, sel && { color: NAVY }]}>{formatDisplayDate(item.date)}</Text>
                      <Text style={s.txExpiry}>Expires {formatDisplayDate(item.expireOn)}</Text>
                    </View>
                    <View style={s.txBadge}>
                      <Text style={s.txBadgeText}>{remaining} day{remaining !== 1 ? "s" : ""}</Text>
                    </View>
                    <View style={[s.txCheck, sel && s.txCheckActive]}>
          ...
          <View style={{ padding: 16 }}>
            <TouchableOpacity onPress={onClose} style={[s.btn, { marginTop: 0 }]}>
              <Text style={s.btnText}>Done{selectedCount > 0 ? ` (${selectedCount})` : ""}</Text>
            </TouchableOpacity>
          </View>
        </View>
      </View>
...
function SelectRow({ value, placeholder, onPress, error }: {
  ...
  return (
    <TouchableOpacity
      onPress={onPress}
      activeOpacity={0.75}
      style={[s.inputWrap, error ? s.inputError : null]}
    >
...
export default function CompoffFormPopup({ onClose, onSuccess }: Props) {
  ...
  const [availForDates, setAvailForDates] = useState<string[]>([])
  const [remarks, setRemarks] = useState("")
  const [errors, setErrors] = useState<FormErrors>({})
  const [submitted, setSubmitted] = useState(false)
  const [showSuccess, setShowSuccess] = useState(false)
  const [successData, setSuccessData] = useState<{ employeeID: string; fromDate: string; toDate: string } | null>(null)
  ...
  const { loading: txLoading, refetch: fetchTx } = useGetRequest<any>({
    ...
    data: [
      ...
      { field: "expireOn", operator: "gte", value: todayStr() },
    ],
    onSuccess: (data) => {
      const raw: CompOffTransaction[] = Array.isArray(data) ? data : (data?.data ?? [])
      setTransactions(
        raw
          .filter(tx => !!tx._id && !!tx.date && !!tx.expireOn && getRemainingCompOff(tx) > 0)
          .sort((a, b) => new Date(a.expireOn).getTime() - new Date(b.expireOn).getTime())
      )
    },
  ...
  useEffect(() => {
    if (employeeId && tenantCode) void fetchTx()
  ...
  const { post, loading: posting } = usePostRequest({
    ...
    onError: () => setErrors(p => ({ ...p, toDate: "Submission failed. Please try again." })),
  })

  const handleSubmit = () => {
    setSubmitted(true)
    const errs = validate({ fromDate, toDate })
    setErrors(errs)
    ...
    const msStr = String(ist.getMilliseconds()).padStart(3, "0")
    const createdOn = `${yyyy}-${mm}-${dd}T${hh}:${mi}:${ss}.${msStr}+05:30`
        ...
        <ScrollView contentContainerStyle={{ flexGrow: 1 }} showsVerticalScrollIndicator={false}>
          <View style={[s.frame, { alignItems: "center", justifyContent: "center", paddingTop: 72 }]}>
            ...
            <Text style={s.successSub}>
              Your comp off request has been submitted successfully and is now pending approval.
            </Text>
            ...
            <View style={s.successCard}>
              ...
              <View style={s.successGrid}>
                ...
                <View style={s.successGridItem}>
                  <Text style={s.successGridLabel}>From Duration</Text>
                  <Text style={s.successGridValue}>{fromDuration}</Text>
                </View>
                <View style={s.successGridItem}>
  ...
  return (
    <View style={s.screen}>
        ...
        <View style={s.frame}>
          ...
          <View style={s.topRow}>
            <View style={s.leftGroup}>
              ...
              <Text style={s.greeting}>Comp Off</Text>
            </View>
          </View>
          ...
          <View style={s.titleRow}>
            ...
            <View style={s.badge}>
              <Text style={s.badgeText}>Comp Off</Text>
            </View>
          </View>
          ...
          <View style={s.card}>
            ...
            <Text style={s.cardSub}>Enter date range, duration, available dates and remarks to submit your comp off request.</Text>
            ...
            {/* Available For Dates */}
            <Label text="Available For Dates" required={false} />
            <TouchableOpacity
              onPress={() => setShowTxPicker(true)}
              style={[s.addDateBtn, { marginBottom: availForDates.length > 0 ? 8 : 0 }]}
            >
              <Plus size={13} color={NAVY} />
              <Text style={s.addDateText}>
                {availForDates.length > 0 ? `${availForDates.length} date${availForDates.length !== 1 ? "s" : ""} selected` : "Select Comp-Off Dates"}
              </Text>
            </TouchableOpacity>
            {availForDates.length > 0 && (
              <View style={{ gap: 6 }}>
                {availForDates.map((d, i) => (
                  <View key={i} style={{ flexDirection: "row", gap: 8, alignItems: "center" }}>
                    <View style={[s.inputWrap, { flex: 1 }]}>
                      <Ionicons name="calendar-outline" size={13} color="#94a3b8" />
                      <Text style={s.inputText}>{formatDisplayDate(d)}</Text>
                    </View>
                    <TouchableOpacity
                      onPress={() => setAvailForDates(prev => prev.filter((_, j) => j !== i))}
                      style={s.removeBtn}
                    >
          ...
          >
            {posting ? (
              ...
              <Text style={s.btnText}>Submit Comp Off Request</Text>
            )}
          </TouchableOpacity>
      ...
      <TransactionPickerModal
        visible={showTxPicker} onClose={() => setShowTxPicker(false)}
        transactions={transactions} loading={txLoading}
        selectedDates={availForDates}
        onToggle={date => setAvailForDates(prev =>
          prev.includes(date) ? prev.filter(d => d !== date) : [...prev, date]
        )}
        onSelectAll={() => setAvailForDates(transactions.map(tx => tx.date))}
        onClearAll={() => setAvailForDates([])}
      />
    </View>
  )
}
...
const s = StyleSheet.create({
  ...
  addDateBtn: { flexDirection: "row", alignItems: "center", gap: 6, paddingVertical: 9, paddingHorizontal: 12, borderRadius: 6, borderWidth: 1, borderColor: NAVY, borderStyle: "dashed" },
  addDateText: { fontFamily: FONT, fontSize: 12, color: NAVY, fontWeight: "600" },
  ...
  sheetSub: { fontFamily: FONT, fontSize: 11, color: "#94a3b8", marginTop: 2 },
  sheetClose: { width: 28, height: 28, borderRadius: 14, backgroundColor: "#f1f5f9", alignItems: "center", justifyContent: "center" },
  txRow: { flexDirection: "row", alignItems: "center", gap: 10, paddingHorizontal: 16, paddingVertical: 13, borderBottomWidth: 1, borderBottomColor: "#f8fafc" },
  txDate: { fontFamily: FONT, fontSize: 13, fontWeight: "700", color: "#0f172a" },
  txExpiry: { fontFamily: FONT, fontSize: 11, color: "#94a3b8", marginTop: 2 },
  txBadge: { borderRadius: 999, backgroundColor: "#eef2ff", paddingHorizontal: 8, paddingVertical: 3 },
  txBadgeText: { fontFamily: FONT, fontSize: 11, fontWeight: "700", color: NAVY },
  txCheck: { width: 20, height: 20, borderRadius: 10, borderWidth: 1.5, borderColor: "#cbd5e1", alignItems: "center", justifyContent: "center", backgroundColor: "#fff" },
  txCheckActive: { borderColor: NAVY, backgroundColor: NAVY },
